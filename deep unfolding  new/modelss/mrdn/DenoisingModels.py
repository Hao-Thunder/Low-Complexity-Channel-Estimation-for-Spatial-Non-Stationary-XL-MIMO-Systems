...
class MRDN(nn.Module):
    def __init__(self, input_channel=65, numofmodules=1, numforrg=2, numofrdb=4, numofconv=6, numoffilters=80, t=1):
        ...
        for i in range(self.numofrdb // (self.numofmodules * self.numforrg)):  # 10 // ( 2 * 4) = 1
            modules.append(GGRDB(self.numofmodules, self.numofkernels, self.nDenselayer, self.numofkernels,
                                    self.numforrg))  # 2,filter,6,filter,4

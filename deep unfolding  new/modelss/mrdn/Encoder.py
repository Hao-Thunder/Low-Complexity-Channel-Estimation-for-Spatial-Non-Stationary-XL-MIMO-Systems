...
class decoder(nn.Module):
    def __init__(self, input_channel, layersize=6, numoffilters=8):
        ...
        self.conv = nn.Sequential(
            *[nn.Sequential(
                nn.Conv2d(1 if i > 0 else numoffilters, 1, kernel_size=3, stride=1, padding=1),
                nn.ReLU()
            ) for i in range(layersize*4)]
        )
...
class Classifier(nn.Module):
    def __init__(self, input_channel, layersize=6, numoffilters=8):
        ...
        self.conv1 = nn.Sequential(
            *[nn.Sequential(
                nn.Conv2d(numoffilters*2 if i > 0 else 1, numoffilters*2, kernel_size=3, stride=1, padding=1),
                nn.ReLU()
            ) for i in range(layersize*4)]
        )

...
from mymodel1 import CustomDataset, VBInet, Gradnet31, PGDnet
...
active_index = (grid_active_index[:, 0] - 1) * Mz + grid_active_index[:, 1]
print(h_set.shape)

Phi = np.load(f'dataset/Phi_10.npy')
...
# VBI_scheduler = optim.lr_scheduler.StepLR(VBI_optimizer, step_size=100, gamma=0.5)

# Grad_net = Gradnet31(Iter_grad=1, depths=[1, 1, 1, 1], dims=[64, 128, 256, 512])
# Grad = nn.DataParallel(Grad_net, device_ids=device_ids).cuda()
# Grad_optimizer = torch.optim.Adam(Grad.parameters(), lr=LR_Grad)
# Grad_scheduler = optim.lr_scheduler.StepLR(Grad_optimizer, step_size=200, gamma=0.5)
...
# PGD_scheduler = optim.lr_scheduler.StepLR(PGD_optimizer, step_size=250, gamma=0.1)
...
# save_dir = f"dataset/"
# np.save(os.path.join(save_dir, "F_white_pinv.npy"), F_white_pinv.cpu().detach().numpy())

# F_white_pinv_one = np.load(f'dataset/F_white_pinv.npy')
# F_white_pinv1 = torch.from_numpy(F_white_pinv_one).repeat(BATCH_SIZE, 1, 1).cuda()
#
# print(F_white_pinv.dtype)
# print(F_white_pinv1.dtype)
#
# torch.max(torch.abs(F_white_pinv - F_white_pinv1))
...
for epoch in range(EPOCH):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        # z_white = M_FPN_OAMP_pinv @ y_hybrid
        ...
        MSE_VBI_1 = Loss(h_torch_real.cuda(), h_est)

        VBI3_optimizer.zero_grad()
        MSE_VBI_1.backward()  # 计算model1的梯度
        VBI3_optimizer.step()
        ...
        #################################################################################
        # F_torch2 = F_torch.clone().detach().cuda()
        # z_torch2 = z_torch_VBI.clone().detach().cuda()
        #
        # grad_Q_to_g10, grad_Q_to_g20, index_amp0 = function_grad_preparation11(y_torch, F_torch2, z_torch2,
        #                                                                        polar_grid_torch, Mx, Mz,
        #                                                                        A_torch, lambda_, d, M, nGrid, sigma2,
        #                                                                        nPath)
        #
        # grad_Q_to_g11 = grad_Q_to_g10.clone().detach().cuda()
        # grad_Q_to_g22 = grad_Q_to_g20.clone().detach().cuda()
        # index_ampx = index_amp0.clone().detach().cuda()
        # F_update_grad, grid_update = Grad(y_torch, F_torch2, z_torch2, polar_grid_torch, Mx, Mz, A_torch_real, lambda_,
        #                                   d, M, nGrid, sigma2, nPath, grad_Q_to_g11, grad_Q_to_g22, index_ampx)
        #
        ...
        # F_PGD = F_torch.clone().detach().cuda()
        # z_PGD = z_torch_VBI.clone().detach().cuda()
        ...
        # MSE_PGD_1.backward()  # 计算model1的梯度
        # PGD_optimizer.step()
        #
        ...
        if Step % 10 == 0:
            ...
            # current_lr = PGD11_optimizer.param_groups[0]['lr']
        ...
        if batch_idx == 0:
            print('---------------------------------------Test Start-------------------------------------------')
            print("NMSE", final_nmse_VBI_1)
            print('---------------------------------------Test end-------------------------------------------')

        mse_value = MSE_VBI_1.item()
        nmse_value = final_nmse_VBI_1.item()
...
torch.save(VBI3.state_dict(), './model_pt/VBI3_hybrid_kapa=0_5_L4_Q32_i1.pt')  # test1
# torch.save(Grad.state_dict(), './model_pt/model3_Grad_hybrid_test_2.pt')
# torch.save(PGD.state_dict(), './model_pt/test/model3_PGD1.pt')

run_name = "VBI3_hybrid_kapa=0_5_L4_Q32_i1"
